...
int main_encode(int argc, char **argv)
{
	...
	cli_opt_t opt = {0};
	int ret = 0;
//    __android_log_print(ANDROID_LOG_INFO,TAG,"In the main_encode");
	...
	// 	//20-22,23-25 x264_lambda_tab ÏàÍ¬
...
//		__android_log_print(ANDROID_LOG_INFO,TAG,"In the main_encode");
	...
	SetConsoleTitle( originalCTitle );
  //  __android_log_print(ANDROID_LOG_INFO,TAG,"In the main_encode,after the encode");
	return ret;
}
...
static int encode( x264_param_t *param, cli_opt_t *opt )
{
	...
	int 	i_start_new,i_end_new,i_Encode_Frame=0;
...
#   define  MAX_PTS_WARNING 3 /* arbitrary */
	...
	FILE *time;
	time=fopen("//sdcard//work//time.txt","wb");
    ...
    for( i_frame = 0, i_frame_output = 0; !b_ctrl_c && (i_frame < param->i_frame_total || !param->i_frame_total); i_frame++ )
    {
		...
		i_start_new = x264_mdate();//clock();
        i_frame_size = encode_frame( h, opt->hout, &pic, &last_dts );
		i_end_new= x264_mdate();//
		i_Encode_Frame +=i_end_new-i_start_new;
	...
	double time_span=0;
	time_span=(double)i_Encode_Frame/CLOCKS_PER_SEC;
	fprintf(time,"%f",time_span);
	fclose(time);
...
*/
		double fps = (double)i_frame_output * (double)1000 /
			(double)( i_Encode_Frame );
        fprintf( stderr, "encoded %d frames, %.2f fps, %.2f kb/s \n", i_frame_output, fps,
                 (double) i_file * 8 / ( 1000 * duration ));

		fp = fopen("//sdcard//work//EncFPS.txt", "a+");
		fprintf(fp, "%.2f\n", fps);
		fclose(fp);
		fp = fopen("//sdcard//work//Encode_Frame.txt","a+");
		fprintf(fp,"%d\n",i_Encode_Frame);
		fclose(fp);
    }

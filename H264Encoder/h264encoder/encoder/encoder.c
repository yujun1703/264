...
#define LOG_OUT	0
extern int try_drop_frame( x264_t *h );
extern int try_drop_frame_MAD( x264_t *h); // mdou
#if 0
...
//added by fred for sp gf
int inter_frame_NalRefIdc[4] = 
{NAL_PRIORITY_DISPOSABLE,   //level_0, short, unref
NAL_PRIORITY_HIGH,         //level_1, short ,ref
NAL_PRIORITY_HIGHEST,       //levle_2, long

NAL_PRIORITY_HIGH           //level_1_p
};
//added by fred for sp gf end
...
static void x264_frame_dump( x264_t *h )
{
	...
	//sprintf(name, "d:\\jmdebug\\fdec %d.yuv", id-1);
	//id++;
	sprintf(name, "d:\\jmdebug\\fdec.yuv");
	if ( 0 == id )
	{
		mode_id = 0;
	}
	...
	//fseek( f, (uint64_t)h->fdec->i_frame * frame_size, SEEK_SET );//zhs no seek
                                    ...
                                    int i_idr_pic_id, int i_frame, int i_qp )
{
	...
	//added by fred: reorder for gf
	sh->b_ref_pic_list_reordering[0] = 0;
	sh->b_ref_pic_list_reordering[1] = 0;

	if ( tp_lvl_2 == h->fenc->curr_level && !IS_X264_TYPE_I( h->fenc->i_type ) )
	{
		int index;
		int have_long_ref = 0;
		...
		{
			if ( tp_lvl_2 == h->frames.reference[i]->curr_level )
			{
				index = i;
				have_long_ref = 1;
				break;
...
static void x264_slice_header_write( bs_t *s, x264_slice_header_t *sh, int i_nal_ref_idc )
{
    ...
    if( i_nal_ref_idc != 0 )
    {
        if( sh->i_idr_pic_id >= 0 )
        {
			...
			bs_write1( s, 1 );  /* by fred I must be long term */
        }
        else
        {
            ...
            {
				//added by fred: write all possible mmco
				for( int i = 0; i < sh->i_mmco_command_count; i++ )
				{
					bs_write_ue( s, sh->mmco[i].opcode ); 
					switch ( sh->mmco[i].opcode )
					{
					case MMCO_SHORT2UNUSED:// 1
						bs_write_ue( s, sh->mmco[i].short_pic_num - 1 );//
						break;
					case MMCO_LONG2UNUSED:// 2
						bs_write_ue( s, sh->mmco[i].long_arg );
						break;
					case MMCO_SHORT2LONG:// 3
						bs_write_ue( s, sh->mmco[i].short_pic_num - 1 );//
						bs_write_ue( s, sh->mmco[i].long_arg );
						break;
					case MMCO_SET_MAX_LONG:// 4
						bs_write_ue( s, sh->mmco[i].long_arg );//max_
						break;
					case MMCO_LONG:// 6
						bs_write_ue( s, sh->mmco[i].long_arg );
                        break;
				...
				//added by fred write all possible mmco end
            }
        }
    }
...
static int x264_bitstream_check_buffer( x264_t *h )
{
        ...
        (h->out.bs.p_end - h->out.bs.p < max_row_size) )
    {
        ...
        CHECKED_MALLOC(uint8_t *, h->out.p_bitstream, h->out.i_bitstream );
...
static int x264_validate_parameters( x264_t *h, int b_open )
{
	...
	// mdou if RC mode && target kbps<=0,error
	if ( (h->param.rc.i_rc_method) && (h->param.rc.i_bitrate<=0))
	{
		x264_log(h, X264_LOG_ERROR, "invalid target bitrate (%d)\n", h->param.rc.i_bitrate);
		return -1;
	}//*/
    ...
    h->param.rc.f_rf_constant = (float)x264_clip3f( h->param.rc.f_rf_constant, -QP_BD_OFFSET, 51 );
    h->param.rc.f_rf_constant_max = (float)x264_clip3f( h->param.rc.f_rf_constant_max, -QP_BD_OFFSET, 51 );
    ...
    {
        h->param.rc.i_qp_constant = (int)( h->param.rc.f_rf_constant + QP_BD_OFFSET );
    ...
    {
        float qp_p = (float)h->param.rc.i_qp_constant;
        float qp_i = (float)(qp_p - 6*log2f( h->param.rc.f_ip_factor ));
        float qp_b = (float)(qp_p + 6*log2f( h->param.rc.f_pb_factor ));
    ...
    h->param.rc.f_vbv_buffer_init = (float)x264_clip3f( h->param.rc.f_vbv_buffer_init, 0, 2000000 );
    ...
    float fps = h->param.i_fps_num > 0 && h->param.i_fps_den > 0 ? (float) h->param.i_fps_num / h->param.i_fps_den : 25.0f;
    if( h->param.i_keyint_min == X264_KEYINT_MIN_AUTO )
        h->param.i_keyint_min = (int)( X264_MIN( h->param.i_keyint_max / 10, fps ));
    ...
    h->param.rc.i_lookahead = x264_clip3( h->param.rc.i_lookahead, 0, X264_LOOKAHEAD_MAX );
    {
        ...
        h->param.rc.i_lookahead = (int)(X264_MIN( h->param.rc.i_lookahead, X264_MAX( h->param.i_keyint_max, bufsize*fps ) ));
    }
    ...
    h->param.rc.f_qcompress = (float)x264_clip3f( h->param.rc.f_qcompress, 0.0, 1.0 );
    ...
    h->param.rc.f_aq_strength = (float)x264_clip3f( h->param.rc.f_aq_strength, 0, 3 );
    ...
    h->param.analyse.f_psy_rd = (float)x264_clip3f( h->param.analyse.f_psy_rd, 0, 10 );
    h->param.analyse.f_psy_trellis = (float)x264_clip3f( h->param.analyse.f_psy_trellis, 0, 10 );
...
//    int satd = !h->mb.b_lossless && h->param.analyse.i_subpel_refine > 1;
    int satd = !h->mb.b_lossless && h->param.analyse.i_cmplx_level > 0;
...
//modify by fred, init list, with short then long
static inline void x264_reference_build_list( x264_t *h, int i_poc )
{
	int b_ok;
	int short_count_list0 = 0;
	...
	for( int i = 0; h->frames.reference[i]; i++ )
	{
		if( h->frames.reference[i]->b_corrupt )
			continue;
		if( h->frames.reference[i]->i_poc < i_poc && IS_SHORT_REFFRAME( h->frames.reference[i]->curr_level ))
			h->fref[0][h->i_ref[0]++] = h->frames.reference[i];
		else if( h->frames.reference[i]->i_poc > i_poc )
			h->fref[1][h->i_ref[1]++] = h->frames.reference[i];
	}
	//zhs add short-term to list0 end
	...
	//zhs add long-term to list0 and order
	short_count_list0 = h->i_ref[0];
	for( int i = 0; h->frames.reference[i]; i++ )
	{
		if ( IS_LONG_REFFRAME( h->frames.reference[i]->curr_level ) )
		{
			h->fref[0][h->i_ref[0]++] = h->frames.reference[i];            
		}
	}
	...
	{
		b_ok = 1;
		for( int i = short_count_list0; i < h->i_ref[0] - 1; i++ )
		{
	...
	//zhs add long-term to list0 and order end
	...
	//x264_reference_check_reorder( h );//zhs
...
/*
added by fred
int i_curr_frame_num:
int apply_on_dpb: 0 just mmco, 1 apply on dpb
*/
static inline int x264_get_mmco( x264_t *h , int i_curr_frame_num, int apply_on_dpb )
{
	int mmco_count, ret;
	x264_frame_t *tmp_dpb[X264_REF_MAX+2];//get a copy of dpb
	x264_frame_t **p_dpb;
	...
	//idr
	if ( X264_TYPE_IDR == h->fenc->i_type )
	{
		ret = 0;
		goto exit;
	}
	...
	{
		p_dpb = tmp_dpb;
		memcpy( tmp_dpb, h->frames.reference, sizeof(tmp_dpb));
	}
	else
	{
		p_dpb = h->frames.reference;
	}

	temporal_levle_idc current_level = h->fenc->curr_level;
	temporal_levle_idc cur_ref_level = h->fenc->ref_level;

	if ( tp_lvl_2 == current_level )
	{
		if ( 1 == apply_on_dpb )
		{            
        ...
        {
            h->sh.mmco[ mmco_count++ ].opcode = MMCO_RESET;// mmco=5

    		h->sh.mmco[ mmco_count ].opcode = MMCO_LONG;
    		h->sh.mmco[ mmco_count++ ].long_arg = 0;        
        }
        
		ret = 0;
		goto exit;
	}
	else if ( tp_lvl_1 == current_level )// for sp or pnosp
	{
		int index = 0;
		int have_short_ref = 0;

		for ( int i = 0; p_dpb[i]; i++ )
		{
			if ( tp_lvl_1 == p_dpb[i]->curr_level )
			{
				index = i;
				have_short_ref = 1;
				break;
		...
		if ( 1 == have_short_ref )
		{
			if ( 1 == apply_on_dpb )
			{
				x264_frame_push_unused( h, x264_frame_shift( p_dpb + index));
			}
            else
            {
                h->sh.mmco[ mmco_count ].opcode = MMCO_SHORT2UNUSED;
			    h->sh.mmco[ mmco_count++].short_pic_num = i_curr_frame_num - p_dpb[ index ]->i_frame_num;
            }
		}
	}
	else
...
exit:
	h->sh.i_mmco_command_count = mmco_count;
	return ret;
}
...
static inline int x264_reference_update( x264_t *h )
{
    ...
    if ( tp_lvl_0 == h->fdec->curr_level )
        x264_frame_push_unused(h, h->fdec);        
    else
	x264_frame_push( h->frames.reference, h->fdec );
    
    int ref_frame_count = h->sps->i_num_ref_frames;
    if ( 2 >= ref_frame_count )
        return -1;
    
	if( h->frames.reference[ ref_frame_count ] )//by fred: buffer full
		x264_frame_push_unused( h, x264_frame_shift( &(h->frames.reference[ref_frame_count - 1]) ) );
...
static inline void x264_reference_hierarchy_reset( x264_t *h )
{
	...
	if( h->param.i_bframe_pyramid != X264_B_PYRAMID_STRICT && !b_hasdelayframe && -1 == h->frames.i_poc_last_open_gop )
        return;
...
static inline void x264_slice_init( x264_t *h, int i_nal_type, int i_global_qp )
{
    ...
    else
    {
		...
		//added by fred
		assert( h->fref[0][0]->curr_level == h->fenc->ref_level );
		//printf("ref-poc: %d\t cur-poc %d\n", h->fref[0][0]->i_poc/2, h->fenc->i_poc/2);
        
		h->sh.i_num_ref_idx_l0_active = h->i_ref[0] <= 0 ? 1 : h->i_ref[0];

        // set num_ref_idx_l0_active for gf
		if ( tp_lvl_2 == h->fenc->curr_level )
		{
			h->sh.i_num_ref_idx_l0_active = 1;
		}
        //fred set num_ref_idx_l0_active for gf end
    ...
    if( X264_TYPE_BREF == h->fenc->i_type && h->param.b_bluray_compat && h->sh.i_mmco_command_count )
    {
        h->b_sh_backup = 1;
        h->sh_backup = h->sh;
    }
...
#if RC_MAD
	int slice_header_bits = bs_pos(&h->out.bs);
    x264_slice_header_write( &h->out.bs, &h->sh, h->i_nal_ref_idc );
	slice_header_bits = bs_pos(&h->out.bs) - slice_header_bits;
	rc_store_slice_header_bits(h,slice_header_bits);
#else
    x264_slice_header_write( &h->out.bs, &h->sh, h->i_nal_ref_idc );
#endif
    if( h->param.b_cabac )
...
{    
    ...
    /* dispatch */
    for( int i = 0; i < h->param.i_threads; i++ )
        x264_threadpool_run( h->threadpool, (void *(__cdecl *)(void *))x264_slices_write, h->thread[i] );
    /* wait */
    for( int i = 0; i < h->param.i_threads; i++ )
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    CHECKED_MALLOCZERO(x264_t *, h, sizeof(x264_t) );
    ...
    CHECKED_MALLOCZERO( x264_frame_t **,h->frames.framelist, (h->frames.i_delay + 5 + h->i_thread_frames + X264_REF_MAX + 4) * sizeof(x264_frame_t *) );
    CHECKED_MALLOCZERO( x264_frame_t **,h->frames.unused[0], (h->frames.i_delay + 5) * sizeof(x264_frame_t *) );
    /* Allocate room for max refs plus a few extra just in case. */
    CHECKED_MALLOCZERO( x264_frame_t **, h->frames.unused[1], (h->i_thread_frames + X264_REF_MAX + 4) * sizeof(x264_frame_t *) );
    CHECKED_MALLOCZERO( x264_frame_t **, h->frames.current, (h->param.i_sync_lookahead + h->param.i_bframe
                        + h->i_thread_frames + 3) * sizeof(x264_frame_t *) );
    if( h->param.analyse.i_weighted_pred > 0 )
        CHECKED_MALLOCZERO( x264_frame_t **, h->frames.blank_unused, h->i_thread_frames * 4 * sizeof(x264_frame_t *) );
    ...
    CHECKED_MALLOC(uint8_t *, h->nal_buffer, h->nal_buffer_size );

    if( h->param.i_threads > 1 &&
        x264_threadpool_init( &h->threadpool, h->param.i_threads, (void (__cdecl *)(void *))x264_encoder_thread_init, h ) )
        goto fail;

    h->thread[0] = h;
    for( int i = 1; i < h->param.i_threads + !!h->param.i_sync_lookahead; i++ )
        CHECKED_MALLOC( x264_t *,h->thread[i], sizeof(x264_t) );
    ...
    for( int i = 0; i < h->param.i_threads; i++ )
    {
        ...
        CHECKED_MALLOC(uint8_t *, h->thread[i]->out.p_bitstream, h->out.i_bitstream );
        /* Start each thread with room for init_nal_count NAL units; it'll realloc later if needed. */
        CHECKED_MALLOC(x264_nal_t *, h->thread[i]->out.nal, init_nal_count*sizeof(x264_nal_t) );
...
void x264_encoder_set_period(x264_t* h, int idr_gop, int gf_gop, int sp_gop)
{
	x264_ratecontrol_reset_gop(h);

	h->param.intra_period = idr_gop;
	h->param.gf_period = gf_gop;
	h->param.sp_period = sp_gop;
}

void x264_encoder_reset_rc_buffer(x264_t* h)
{
	x264_ratecontrol_reset_buffer(h);
}

void x264_encoder_set_delaytime(x264_t *h, int sec)
{
    x264_ratecontrol_set_delaytime(h, sec);
}

//added by fred update pic_num in dpb
void update_short_frame_pic_num( x264_t *h )
{
	x264_frame_t** dpb = (x264_frame_t**)h->frames.reference;
	int max_frame_num = ((1<<h->sps->i_log2_max_frame_num));
	int curr_frame_num = h->i_frame_num % max_frame_num;

	for ( int i = 0; dpb[i]; i++ )
	{
		int frame_num_warp;
		int x_frame_num = dpb[i]->i_frame_num % max_frame_num;

		if ( x_frame_num > curr_frame_num )
		{
			frame_num_warp = x_frame_num - max_frame_num;
		}
		else
		{
			frame_num_warp = x_frame_num;
		}

		dpb[i]->pic_num = frame_num_warp;
	}
}
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    //fred
	if (h->param.rc.use_drop_frame)  // mdou
	{
#if RC_MAD
		if (h->param.rc.use_MAD)
		{
			if (try_drop_frame_MAD( h ))
			{
				return 0;
			}
...
// 		}
	...
	//added by fred
	h->fdec->curr_level = h->fenc->curr_level;
	h->fdec->ref_level = h->fenc->ref_level;
	...
	else //added by fred: for sp gf  
	{
		assert ( X264_TYPE_P == h->fenc->i_type );

		i_nal_type    = NAL_SLICE;
		i_nal_ref_idc = inter_frame_NalRefIdc[ (int)h->fenc->curr_level ];        
    ...
    if( h->param.b_intra_refresh )
    {
        if( IS_X264_TYPE_I( h->fenc->i_type ) )
        {
            ...
            h->fdec->f_pir_position = (float)h->mb.i_mb_width;
        }
        else if( h->fenc->i_type == X264_TYPE_P )
        {
            ...
            h->fdec->i_pir_start_col = (int)(h->fdec->f_pir_position+0.5);
            h->fdec->f_pir_position += increment * pocdiff;
            h->fdec->i_pir_end_col = (int)( h->fdec->f_pir_position+0.5);
	...
	// added by fred gf use mmco = 5, so set frame_num = 0; then get mmco option 
	if ( tp_lvl_2 == h->fenc->curr_level && X264_TYPE_IDR != h->fenc->i_type )
	{
		h->i_frame_num = 0;
	}
    
	x264_get_mmco( h, h->i_frame_num, 0 );
    //fred gf end
    ...
    if( h->i_thread_frames > 1 )
    {
        x264_threadpool_run( h->threadpool,(void *(__cdecl *)(void *))x264_slices_write, h );
        h->b_thread_active = 1;
    }
    ...
    // added by fred apply mmco on dpb    
// 	x264_get_mmco( h , (h->i_nal_ref_idc != NAL_PRIORITY_DISPOSABLE )? h->i_frame_num - 1 : h->i_frame_num, 1 );
...
// 	}
	...
	if (h->param.rc.b_drop_frame)
	{
		...
		if( h->i_nal_ref_idc != NAL_PRIORITY_DISPOSABLE )
			h->i_frame_num--;	
		x264_lookahead_restore_frames( thread_current );
	}
	else
	{
		// added by fred apply mmco on dpb    
		...
		x264_lookahead_update_frames( thread_current );
	}
...
int x264_encoder_reset_esolution( x264_t *h, int width, int height)
{
...
//     h->sps->i_mb_width = ( width + 15 ) / 16;
//     h->sps->i_mb_height= ( height + 15 ) / 16;
    x264_frame_reset_size(h);   
...
#define SUM3b(p,o) (p[SLICE_TYPE_I][o] + p[SLICE_TYPE_P][o] + p[SLICE_TYPE_B][o])
...
#if LOG_OUT
			FILE *fp_ssim = fopen("x264_SSIM.txt", "a");
			fprintf(fp_ssim, "%.4f\n",ssim );
			fclose(fp_ssim);
#endif
        }
        if( h->param.analyse.b_psnr )
        {
            x264_log( h, X264_LOG_INFO,
                      ...
                      f_bitrate );
#if LOG_OUT
			FILE *fp_psnr = fopen("x264_PSNR.txt", "a");
			fprintf(fp_psnr, "%.2f\t%.2f\t%.2f\t%.2f\n",f_bitrate,SUM3( h->stat.f_psnr_mean_y ) / duration,
				SUM3( h->stat.f_psnr_mean_u ) / duration,
				SUM3( h->stat.f_psnr_mean_v ) / duration );
			fclose(fp_psnr);
#endif
...
unsigned int get_h264encoder_version()
{
	return H264ENCODERVER;
}
float get_avg_qp( x264_t *h )
{
    if(!h)
        return 0.0;
    return h->fdec->f_qp_avg_aq;
}
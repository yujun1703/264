...
static void x264_lookahead_update_backup_nonb( x264_t *h, x264_frame_t *new_nonb )
{
	if( h->lookahead->backup_nonb )
		x264_frame_push_unused( h, h->lookahead->backup_nonb );
	h->lookahead->backup_nonb = new_nonb;
	new_nonb->i_reference_count++;
}


static void x264_lookahead_update_last_gf( x264_t *h, x264_frame_t *new_nonb )
{
	if( h->lookahead->last_gf )
		x264_frame_push_unused( h, h->lookahead->last_gf );
	h->lookahead->last_gf = new_nonb;
	new_nonb->i_reference_count++;
}


static void x264_lookahead_update_last_sp( x264_t *h, x264_frame_t *new_nonb )
{
	if( h->lookahead->last_sp )
		x264_frame_push_unused( h, h->lookahead->last_sp );
	h->lookahead->last_sp = new_nonb;
	new_nonb->i_reference_count++;
}
...
int x264_lookahead_init( x264_t *h, int i_slicetype_length )
{
    ...
    CHECKED_MALLOCZERO(x264_lookahead_t *, look, sizeof(x264_lookahead_t) );
    ...
    *look_h = *h;
    ...
    if( x264_pthread_create( &look->thread_handle, NULL, (void *(__cdecl *)(void *))x264_lookahead_thread, look_h ) )
        goto fail;
...
void x264_lookahead_get_frames( x264_t *h )
{
    ...
    {   /* We are not running a lookahead thread, so perform all the slicetype decide on the fly */
		...
		if (h->lookahead->next.list[0]->curr_level != tp_lvl_0)
		{
        x264_lookahead_update_last_nonb( h, h->lookahead->next.list[0] );
		}
...
void x264_lookahead_restore_frames( x264_t *h )
{
	if (h->lookahead->backup_nonb)
		x264_lookahead_update_last_nonb( h, h->lookahead->backup_nonb );
}

void x264_lookahead_update_frames( x264_t *h )
{
	if ( h->fenc->curr_level == tp_lvl_2)
	{
		x264_lookahead_update_last_gf( h, h->fenc );
		if (h->param.sp_period)  // use sp
		{
			x264_lookahead_update_last_sp( h, h->fenc );
		}
	}
	else if ( h->fenc->curr_level == tp_lvl_1 )
	{
		x264_lookahead_update_last_sp( h, h->fenc );
	}
	
	if (h->fenc->curr_level != tp_lvl_0)
	{
		x264_lookahead_update_backup_nonb( h, h->fenc );
	}
}

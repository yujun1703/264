...
static void x264_mb_encode_i16x16( x264_t *h, int p, int i_qp )
{
	...
	if (h->param.rc.use_MAD)
	{
		...
		{
			{
				sum += abs(p_dst_1[0] - p_src_1[0]);
				sum += abs(p_dst_1[1] - p_src_1[1]);
				sum += abs(p_dst_1[2] - p_src_1[2]);
				sum += abs(p_dst_1[3] - p_src_1[3]);
				sum += abs(p_dst_1[4] - p_src_1[4]);
				sum += abs(p_dst_1[5] - p_src_1[5]);
				sum += abs(p_dst_1[6] - p_src_1[6]);
				sum += abs(p_dst_1[7] - p_src_1[7]);
				sum += abs(p_dst_1[8] - p_src_1[8]);
				sum += abs(p_dst_1[9] - p_src_1[9]);
				sum += abs(p_dst_1[10] - p_src_1[10]);
				sum += abs(p_dst_1[11] - p_src_1[11]);
				sum += abs(p_dst_1[12] - p_src_1[12]);
				sum += abs(p_dst_1[13] - p_src_1[13]);
				sum += abs(p_dst_1[14] - p_src_1[14]);
				sum += abs(p_dst_1[15] - p_src_1[15]);
			}
			p_src_1 += FENC_STRIDE;
			p_dst_1 += FDEC_STRIDE;
		}
...
#if RC_MAD
				x264_mb_encode_i4x4( h, p, i, i_qp, i_mode, 1 ,1);
#else
        ...
        else
        {
            ...
            for( int p = 0; p < plane_count; p++ )
            {
				...
				if (h->param.rc.use_MAD)
				{
					...
					{
						{
							sum += abs(p_dst[0] - p_src[0]);
							sum += abs(p_dst[1] - p_src[1]);
							sum += abs(p_dst[2] - p_src[2]);
							sum += abs(p_dst[3] - p_src[3]);
							sum += abs(p_dst[4] - p_src[4]);
							sum += abs(p_dst[5] - p_src[5]);
							sum += abs(p_dst[6] - p_src[6]);
							sum += abs(p_dst[7] - p_src[7]);
							sum += abs(p_dst[8] - p_src[8]);
							sum += abs(p_dst[9] - p_src[9]);
							sum += abs(p_dst[10] - p_src[10]);
							sum += abs(p_dst[11] - p_src[11]);
							sum += abs(p_dst[12] - p_src[12]);
							sum += abs(p_dst[13] - p_src[13]);
							sum += abs(p_dst[14] - p_src[14]);
							sum += abs(p_dst[15] - p_src[15]);
						}
						p_src += FENC_STRIDE;
						p_dst += FDEC_STRIDE;
					}
...
static ALWAYS_INLINE int x264_macroblock_probe_skip_internal( x264_t *h, int b_bidir, int plane_count, int chroma )
{
    ...
    for( int p = 0; p < plane_count; p++ )
    {
		...
		if (h->param.rc.use_MAD)
		{
			...
			{
				{
					sum += abs(fdec[0] - fenc[0]);
					sum += abs(fdec[1] - fenc[1]);
					sum += abs(fdec[2] - fenc[2]);
					sum += abs(fdec[3] - fenc[3]);

					sum += abs(fdec[4] - fenc[4]);
					sum += abs(fdec[5] - fenc[5]);
					sum += abs(fdec[6] - fenc[6]);
					sum += abs(fdec[7] - fenc[7]);

					sum += abs(fdec[8] - fenc[8]);
					sum += abs(fdec[9] - fenc[9]);
					sum += abs(fdec[10] - fenc[10]);
					sum += abs(fdec[11] - fenc[11]);

					sum += abs(fdec[12] - fenc[12]);
					sum += abs(fdec[13] - fenc[13]);
					sum += abs(fdec[14] - fenc[14]);
					sum += abs(fdec[15] - fenc[15]);
				}
				fenc += FENC_STRIDE;
				fdec += FDEC_STRIDE;
			}

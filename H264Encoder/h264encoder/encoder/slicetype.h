...
static void x264_weights_analyse( x264_t *h, x264_frame_t *fenc, x264_frame_t *ref, int b_lookahead )
{
    ...
    for( int plane = 0; plane <= 2 && !( plane && ( !weights[0].weightfn || b_lookahead ) ); plane++ )
    {
        ...
        int found;
        float fenc_var = (float)(  fenc->i_pixel_ssd[plane] + !ref->i_pixel_ssd[plane]);
        float ref_var  =  (float)( ref->i_pixel_ssd[plane] + !ref->i_pixel_ssd[plane]);
        float guess_scale = (float)( sqrtf( fenc_var / ref_var ));
        ...
        cur_offset = (int)( fenc_mean - ref_mean * minscale / (1 << mindenom) + 0.5f * b_lookahead );
                                      ...
                                      int b_intra_penalty )
{
    ...
    else
    {
        ...
        i_score = frames[b]->i_cost_est[b-p0][p1-b];
        if( b != p1 )
            i_score = (int)( (uint64_t)i_score * 100 / (120 + h->param.i_bframe_bias) );
        else
    ...
    if( b_intra_penalty )
    {
        ...
        int nmb = NUM_MBS;
        i_score += (int)( (uint64_t)i_score * frames[b]->i_intra_mbs[b-p0] / (nmb * 8) );
    }
    return i_score;
}
...
static void x264_macroblock_tree_propagate( x264_t *h, x264_frame_t **frames, float average_duration, int p0, int p1, int b, int referenced )
{
    ...
    x264_emms();
    float fps_factor = (float)( CLIP_DURATION(frames[b]->f_duration) / CLIP_DURATION(average_duration) );
...
void x264_slicetype_analyse( x264_t *h, int keyframe )
{
	...
	if (h->lookahead->next.list[0]->curr_level == tp_lvl_2)
	{
		frames[0] = h->lookahead->last_gf;
	}
	else if (h->param.sp_period)  // use sp
	{
		frames[0] = h->lookahead->last_sp;
	}
	else  
	{
...
void x264_slicetype_decide( x264_t *h )
{
    ...
    for( int i = 0; i < h->lookahead->next.i_size; i++ )
    {
        ...
        h->lookahead->next.list[i]->f_duration = (float)( (double)h->lookahead->next.list[i]->i_duration
...
// 		frames[0] = h->lookahead->last_nonb;
		if (h->lookahead->next.list[bframes]->curr_level == tp_lvl_2)
		{
			frames[0] = h->lookahead->last_gf;
		}
		else if (h->param.sp_period)  // use sp
...
int x264_rc_analyse_slice( x264_t *h )
{
    ...
    {
        int ip_factor = (int)( 256 * h->param.rc.f_ip_factor ); /* fix8 */

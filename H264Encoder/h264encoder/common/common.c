...
#ifdef X264_MEMORY_MONITOR
void * x264_MY_malloc( int size )
{
 	static int x264_MY_malloc_time = 0;
	void * current_p;
	
	current_p = malloc(size);
	if(current_p)
	{
    	x264_MY_malloc_time++;
        
    	printf("Malloc\t%d\t%10d\n",  x264_MY_malloc_time, current_p);
	}
    return current_p;
}

void x264_MY_free( void * p)
{	
	static int x264_MY_free_time = 0;
...
void* MY_realloc(void* p, size_t size)
{
    ...
    p = NULL;
    p = x264_MY_malloc( size );
    
    return p;
}
#else

#define x264_MY_malloc       malloc
#define x264_MY_free	        free
...
{    
    ...
    param->i_threads = 1;
	param->b_sliced_threads = 1;  // added by timlong
    ...
    /* Encoder parameters */
    param->i_frame_reference = 3;//zhs
    param->i_keyint_max = X264_KEYINT_MAX_INFINITE;
    param->i_keyint_min = X264_KEYINT_MIN_AUTO;
    param->i_bframe = 0;
    param->i_scenecut_threshold = 40;
    param->i_bframe_adaptive = X264_B_ADAPT_NONE;//zhs
    param->i_bframe_bias = 0;
    param->i_bframe_pyramid = X264_B_PYRAMID_NONE;//zhs
    ...
    param->rc.f_rate_tolerance = 1.0f;
    ...
    param->rc.i_aq_mode = X264_AQ_VARIANCE;// X264_AQ_NONE X264_AQ_VARIANCE
    ...
    param->rc.f_qcompress = 0.0f;
    param->rc.f_qblur = 0.5f;
    param->rc.f_complexity_blur = 20;
    param->rc.i_zones = 0;
    param->rc.i_lookahead = 0;
    param->rc.b_mb_tree = 0;//by sensong

    //for QQ
    param->rc.vbv_drop_thr = 1.5;
    param->rc.max_drop_num = 1;
    param->rc.drop_start_pos = 2;

    param->em_rc_mode = em_mode_normal;

    /* Log */
    param->pf_log = x264_log_default;
    param->p_log_private = NULL;
    param->i_log_level = X264_LOG_NONE;//X264_LOG_INFO X264_LOG_NONE
    ...
    param->analyse.b_weighted_bipred = 0;//zhs
    param->analyse.i_weighted_pred = X264_WEIGHTP_NONE;//zhs no weighted pred
...
static int x264_param_apply_tune( x264_param_t *param, const char *tune )
{
    ...
    while( s )
    {
        if( !strncasecmp( s, "film", 4 ) )
        {
            ...
            param->analyse.f_psy_trellis = 0.15f;
        }
        else if( !strncasecmp( s, "animation", 9 ) )
        {
            ...
            param->i_deblocking_filter_alphac0 = 1;
            param->i_deblocking_filter_beta = 1;
            param->analyse.f_psy_rd = 0.4f;
            param->rc.f_aq_strength = 0.6f;
            param->i_bframe += 2;
        }
        else if( !strncasecmp( s, "grain", 5 ) )
        {
            ...
            param->analyse.f_psy_trellis = 0.25f;
        ...
        else if( !strncasecmp( s, "stillimage", 5 ) )
        {
            ...
            param->analyse.f_psy_rd = 2.0f;
            param->analyse.f_psy_trellis = 0.7f;
            param->rc.f_aq_strength = 1.2f;
        }
        ...
        else if( !strncasecmp( s, "touhou", 6 ) )
        {
            ...
            param->i_deblocking_filter_alphac0 = -1;
            param->i_deblocking_filter_beta = -1;
            param->analyse.f_psy_trellis = 0.2f;
            param->rc.f_aq_strength = 1.3f;
...
#define OPT2(STR0, STR1) else if( !strcmp( name, STR0 ) || !strcmp( name, STR1 ) )
    ...
    OPT("crf")
    {
        p->rc.f_rf_constant = (float)atof(value);
        p->rc.i_rc_method = X264_RC_CRF;
    }
    OPT("crf-max")
        p->rc.f_rf_constant_max = (float)atof(value);
    OPT("rc-lookahead")
    ...
    OPT("ratetol")
        p->rc.f_rate_tolerance = (float)( !strncmp("inf", value, 3) ? 1e9 : ((float)atof(value)));
    OPT("vbv-maxrate")
        p->rc.i_vbv_max_bitrate = atoi(value);
    OPT("vbv-bufsize")
        p->rc.i_vbv_buffer_size = atoi(value);
    OPT("vbv-init")
        p->rc.f_vbv_buffer_init = (float)atof(value);
    OPT2("ipratio", "ip-factor")
        p->rc.f_ip_factor = (float)atof(value);
    OPT2("pbratio", "pb-factor")
        p->rc.f_pb_factor = (float)atof(value);
    OPT("aq-mode")
        p->rc.i_aq_mode = atoi(value);
    OPT("aq-strength")
        p->rc.f_aq_strength = (float)atof(value);
    OPT("pass")
    {
    ...
    OPT("qcomp")
        p->rc.f_qcompress = (float)atof(value);
    OPT("mbtree")
        p->rc.b_mb_tree = atobool(value);
    OPT("qblur")
        p->rc.f_qblur = (float)atof(value);
    OPT2("cplxblur", "cplx-blur")
        p->rc.f_complexity_blur = (float)atof(value);
    OPT("zones")
...
int x264_picture_alloc( x264_picture_t *pic, int i_csp, int i_width, int i_height )
{
    ...
    {
        int stride = (int)((((int64_t)i_width * x264_csp_tab[csp].width_fix8[i]) >> 8) * depth_factor);
        int plane_size = (int)((((int64_t)i_height * x264_csp_tab[csp].height_fix8[i]) >> 8) * stride);
...
void *x264_malloc( int i_size )
{
    ...
    align_buf = x264_MY_malloc( i_size );
#elif HAVE_MALLOC_H
    align_buf = memalign( 16, i_size );
#else
    uint8_t *buf = (uint8_t *)x264_MY_malloc( i_size + 15 + sizeof(void **) );
    if( buf )
    {

...
#if HAVE_ARMV6 //added by sensong 201305

void x264_predict_4x4_v_neon( pixel *fdec )
{
   asm volatile(
   "sub        %0,%0,#32                       \n\t"
   "mov        r1,#32                          \n\t"
...
void x264_predict_4x4_h_neon( pixel *fdec )
{
   asm volatile(
   "mov        r2,#32                          \n\t"
   "mov        r1,%0                           \n\t"
   ...
   :"+r"(fdec)
   ::"r1","r2","d0","d1","d2","d3","memory"   
   );
}

void x264_predict_4x4_dc_neon( pixel *fdec )
{
   asm volatile(
   "mov        r2,#32                          \n\t"
   "sub        r1,%0,#32                       \n\t"    
...
void x264_intra_satd_x3_4x4_neon( pixel *fenc, pixel *fdec, int res[3] )
{
    x264_predict_4x4_v_neon( fdec );
    res[0] = x264_pixel_satd_4x4_neon( fdec, FDEC_STRIDE, fenc, FENC_STRIDE );
    x264_predict_4x4_h_neon( fdec );
    res[1] = x264_pixel_satd_4x4_neon( fdec, FDEC_STRIDE, fenc, FENC_STRIDE );
    x264_predict_4x4_dc_neon( fdec );
    res[2] = x264_pixel_satd_4x4_neon( fdec, FDEC_STRIDE, fenc, FENC_STRIDE );
}
...
void x264_intra_sad_x3_4x4_neon( pixel *fenc, pixel *fdec, int res[3] )
{
    x264_predict_4x4_v_neon( fdec );
    res[0] = x264_pixel_sad_4x4_neon( fdec, FDEC_STRIDE, fenc, FENC_STRIDE );
    x264_predict_4x4_h_neon( fdec );
    res[1] = x264_pixel_sad_4x4_neon( fdec, FDEC_STRIDE, fenc, FENC_STRIDE );
    x264_predict_4x4_dc_neon( fdec );
    res[2] = x264_pixel_sad_4x4_neon( fdec, FDEC_STRIDE, fenc, FENC_STRIDE );
}

INTRA_MBCMP(sad,  8x8,  dc, h,  v, c,_neon,_neon )      //added by sensong 201404
INTRA_MBCMP(sad, 16x16,  v, h, dc,  ,_neon, _neon )     //added by sensong 201404

                                   ...
                                   uint16_t *inter_costs, uint16_t *inv_qscales, float *fps_factor, int len )
{
    ...
    for( int i = 0; i < len; i++ )
    {
        float intra_cost       = (float)( intra_costs[i] * inv_qscales[i]  );
        float propagate_amount = (float)( propagate_in[i] + intra_cost*fps );
        float propagate_num    = (float)( intra_costs[i] - (inter_costs[i] & LOWRES_COST_MASK) );
        float propagate_denom  = (float)( intra_costs[i] );

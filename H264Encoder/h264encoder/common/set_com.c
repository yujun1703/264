...
int x264_cqm_init( x264_t *h )
{
    ...
    for( int i = 0; i < 4 + num_8x8_lists; i++ )
    {
        ...
        {
            CHECKED_MALLOC(udctcoef (*)[16], h->  quant4_mf[i], (QP_MAX+1)*size*sizeof(udctcoef) );
        ...
        {
            CHECKED_MALLOC(udctcoef (*)[16], h->quant4_bias[i], (QP_MAX+1)*size*sizeof(udctcoef) );
            CHECKED_MALLOC(udctcoef (*)[16], h->quant4_bias0[i], (QP_MAX+1)*size*sizeof(udctcoef) );
        }
    }
    ...
    x264_emms();
    CHECKED_MALLOC(udctcoef (*)[4][64], h->nr_offset_emergency, sizeof(*h->nr_offset_emergency)*(QP_MAX-QP_MAX_SPEC) );
    for( int q = 0; q < QP_MAX - QP_MAX_SPEC; q++ )
        for( int cat = 0; cat < 3 + CHROMA444; cat++ )
        {
            ...
            for( int i = 0; i < size; i++ )
            {
                ...
                nr_offset[i] = (udctcoef)(X264_MIN( bias + 0.5, max ));
            }
        }
...
static int x264_cqm_parse_jmlist( x264_t *h, char *buf, const char *name,

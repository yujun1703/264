...
#if HAVE_MMX
EXTERNFLAG int x264_cpu_cpuid_test( void );
EXTERNFLAG void x264_cpu_cpuid( uint32_t op, uint32_t *eax, uint32_t *ebx, uint32_t *ecx, uint32_t *edx );
EXTERNFLAG void x264_cpu_xgetbv( uint32_t op, uint32_t *eax, uint32_t *edx );
...
//            if( ecx&0x00000080 ) /* Misalign SSE */    // modified by sensong 201308
//            {
...
// armCPUinfo
// |Part(12bits)|Features(20bits)|
#define ARM_FEATURE_SWP               (1<<0)
#define ARM_FEATURE_FASTMULT          (1<<1)
#define ARM_FEATURE_VFP               (1<<2)
#define ARM_FEATURE_EDSP              (1<<3)
#define ARM_FEATURE_NEON              (1<<4)
#define ARM_FEATURE_VFPV3             (1<<5)
#define ARM_FEATURE_VFPV4             (1<<6)
#define ARM_FEATURE_IDIVA             (1<<7)
#define ARM_FEATURE_IDIVT             (1<<8)

#define ARM_920T                      (0x920<<20)
#define ARM_926EJ                     (0x926<<20)
#define ARM_1136                      (0xb36<<20)
#define ARM_1156                      (0xb56<<20)
#define ARM_1176                      (0xb76<<20)
#define ARM_CORTEX_A5                 (0xc05<<20)
#define ARM_CORTEX_A7                 (0xc07<<20)
#define ARM_CORTEX_A8                 (0xc08<<20)
#define ARM_CORTEX_A9                 (0xc09<<20)
#define ARM_CORTEX_A12                (0xc0c<<20)
#define ARM_CORTEX_A15                (0xc0f<<20)



static int get_cpuinfo(unsigned int *armCPUinfo)
{
    ...
    *armCPUinfo = 0;
    ...
    {
        if (strstr(buf, "Features")) {
            if (strstr(buf, " swp "))
                *armCPUinfo |= ARM_FEATURE_SWP;
            if (strstr(buf, " fastmult "))
                *armCPUinfo |= ARM_FEATURE_FASTMULT;
            if (strstr(buf, " vfp "))
                *armCPUinfo |= ARM_FEATURE_VFP;
            if (strstr(buf, " edsp "))
                *armCPUinfo |= ARM_FEATURE_EDSP;
            if (strstr(buf, " neon "))
                *armCPUinfo |= ARM_FEATURE_NEON;
            if (strstr(buf, " vfpv3 "))
                *armCPUinfo |= ARM_FEATURE_VFPV3;
            if (strstr(buf, " vfpv4 "))
                *armCPUinfo |= ARM_FEATURE_VFPV4;
            if (strstr(buf, " idiva "))
                *armCPUinfo |= ARM_FEATURE_IDIVA;
            if (strstr(buf, " idivt "))
                *armCPUinfo |= ARM_FEATURE_IDIVT;
            continue;
        }

        if (strstr(buf, "CPU part"))
        {
        	if (ptr = strstr(buf, "0x"))
        	{
        		char c;
        		int i,shift;
        		shift = 28;
        		...
        		for(i=0;i<3;i++)
        		{
        			...
        			if(c>='0' && c<='9')
        			{
        				*armCPUinfo |= (c-'0')<<shift;
        			}
        			if(c>='a' && c<='f')
        			{
        				*armCPUinfo |= (c-'a'+10)<<shift;
        			}
        			shift -=4;
        		}
        	}
...
uint32_t x264_cpu_detect( void )
{
    ...
    get_cpuinfo(&flags);         // get arm cpu info
    flags |= X264_CPU_ARMV6;
    if(ARM_FEATURE_NEON==(flags &ARM_FEATURE_NEON))
    {
    	flags |= X264_CPU_NEON;
    	if(ARM_CORTEX_A8 != (flags &ARM_CORTEX_A8))
    	{
    		flags |= X264_CPU_FAST_NEON_MRC;    		
    	}
    }
#else
// for ios
...
int x264_cpu_num_processors( void )
{
    ...
    int np = 0;
     /*
    unsigned int bit;
    for(  bit = 0; bit < 8 * sizeof(p_aff); bit++ )

                          ...
                          void (*init_func)(void *), void *init_arg )
{
    ...
    CHECKED_MALLOCZERO(x264_threadpool_t *, pool, sizeof(x264_threadpool_t) );
    *p_pool = pool;
    ...
    CHECKED_MALLOC(x264_pthread_t *, pool->thread_handle, pool->threads * sizeof(x264_pthread_t) );
    ...
    for( int i = 0; i < pool->threads; i++ )
    {
       ...
       CHECKED_MALLOC( x264_threadpool_job_t *,job, sizeof(x264_threadpool_job_t) );
       x264_sync_frame_list_push( &pool->uninit, (x264_frame_t *)job );
    }
    for( int i = 0; i < pool->threads; i++ )
#ifdef WIN32
         if( x264_pthread_create( pool->thread_handle+i, NULL, (void *(__cdecl *)(void *))x264_threadpool_thread, pool ) )
#else
        if( x264_pthread_create( pool->thread_handle+i, NULL, x264_threadpool_thread, pool ) )
#endif
            goto fail;

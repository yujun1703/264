...
static x264_frame_t *x264_frame_new( x264_t *h, int b_fdec )
{
    ...
    CHECKED_MALLOCZERO( x264_frame_t *,frame, sizeof(x264_frame_t) );
    ...
    for( int i = 0; i < h->param.i_bframe + 2; i++ )
        for( int j = 0; j < h->param.i_bframe + 2; j++ )
            CHECKED_MALLOC( int *,frame->i_row_satds[i][j], i_lines/16 * sizeof(int) );
    ...
    if( i_csp == X264_CSP_NV12 || i_csp == X264_CSP_NV16 )
    {
        ...
        CHECKED_MALLOC(pixel *, frame->buffer[1], chroma_plane_size * sizeof(pixel) );
        frame->plane[1] = frame->buffer[1] + frame->i_stride[1] * chroma_padv + PADH;
        if( PARAM_INTERLACED )
        {
            CHECKED_MALLOC(pixel *, frame->buffer_fld[1], chroma_plane_size * sizeof(pixel) );
    ...
    for( int p = 0; p < luma_plane_count; p++ )
    {
        ...
        if( h->param.analyse.i_subpel_refine && b_fdec )
        {
            ...
            CHECKED_MALLOC( pixel *,frame->buffer[p], 4*luma_plane_size * sizeof(pixel) );
            if( PARAM_INTERLACED )
                CHECKED_MALLOC(pixel *, frame->buffer_fld[p], 4*luma_plane_size * sizeof(pixel) );
            for( int i = 0; i < 4; i++ )
            {
        ...
        {
            CHECKED_MALLOC(pixel *, frame->buffer[p], luma_plane_size * sizeof(pixel) );
            if( PARAM_INTERLACED )
                CHECKED_MALLOC(pixel *, frame->buffer_fld[p], luma_plane_size * sizeof(pixel) );
    ...
    {
        CHECKED_MALLOC(int8_t  *, frame->mb_type, i_mb_count * sizeof(int8_t));
        CHECKED_MALLOC(uint8_t *, frame->mb_partition, i_mb_count * sizeof(uint8_t));
        CHECKED_MALLOC(int16_t (*)[2], frame->mv[0], 2*16 * i_mb_count * sizeof(int16_t) );
        CHECKED_MALLOC(int16_t (*)[2], frame->mv16x16, 2*(i_mb_count+1) * sizeof(int16_t) );
        M32( frame->mv16x16[0] ) = 0;
        frame->mv16x16++;
        CHECKED_MALLOC( int8_t  *,frame->ref[0], 4 * i_mb_count * sizeof(int8_t) );
        if( h->param.i_bframe )
        {
            CHECKED_MALLOC(int16_t (*)[2], frame->mv[1], 2*16 * i_mb_count * sizeof(int16_t) );
            CHECKED_MALLOC(int8_t  *, frame->ref[1], 4 * i_mb_count * sizeof(int8_t) );
        }
        else
        {
            frame->mv[1]  = NULL;
        ...
        CHECKED_MALLOC( int   *,frame->i_row_bits, i_lines/16 * sizeof(int) );
        CHECKED_MALLOC( float   *,frame->f_row_qp, i_lines/16 * sizeof(float) );
        CHECKED_MALLOC( float   *, frame->f_row_qscale, i_lines/16 * sizeof(float) );
        if( h->param.analyse.i_me_method >= X264_ME_ESA )
        {
            CHECKED_MALLOC( pixel *,frame->buffer[3],
                            frame->i_stride[0] * (frame->i_lines[0] + 2*i_padv) * sizeof(uint16_t) << h->frames.b_have_sub8x8_esa );
            frame->integral = (uint16_t*)frame->buffer[3] + frame->i_stride[0] * i_padv + PADH;
        }
        if( PARAM_INTERLACED )
            CHECKED_MALLOC(uint8_t *, frame->field, i_mb_count * sizeof(uint8_t) );
    }
    else /* fenc frame */
    {
        if( h->frames.b_have_lowres )
        {
            ...
            CHECKED_MALLOC(pixel *, frame->buffer_lowres[0], 4 * luma_plane_size * sizeof(pixel) );
            for( int i = 0; i < 4; i++ )
            ...
            for( int j = 0; j <= !!h->param.i_bframe; j++ )
                ...
                {
                    CHECKED_MALLOCZERO(int16_t (*)[2], frame->lowres_mvs[j][i], 2*h->mb.i_mb_count*sizeof(int16_t) );
                    CHECKED_MALLOC( int     *,frame->lowres_mv_costs[j][i], h->mb.i_mb_count*sizeof(int) );
                }
            CHECKED_MALLOC( uint16_t *,frame->i_propagate_cost, (i_mb_count+7) * sizeof(uint16_t) );
            for( int j = 0; j <= h->param.i_bframe+1; j++ )
                for( int i = 0; i <= h->param.i_bframe+1; i++ )
                    CHECKED_MALLOC( uint16_t *,frame->lowres_costs[j][i], (i_mb_count+3) * sizeof(uint16_t) );
            frame->i_intra_cost = frame->lowres_costs[0][0];
            memset( frame->i_intra_cost, -1, (i_mb_count+3) * sizeof(uint16_t) );
        }
        if( h->param.rc.i_aq_mode )
        {
            CHECKED_MALLOC( float   *,frame->f_qp_offset, h->mb.i_mb_count * sizeof(float) );
            CHECKED_MALLOC( float   *,frame->f_qp_offset_aq, h->mb.i_mb_count * sizeof(float) );
            if( h->frames.b_have_lowres )
                /* shouldn't really be initialized, just silences a valgrind false-positive in x264_mbtree_propagate_cost_sse2 */
                CHECKED_MALLOCZERO( uint16_t *,frame->i_inv_qscale_factor, (h->mb.i_mb_count+3) * sizeof(uint16_t) );
        }
    }
    ...
    x264_frame_push(h->frames.framelist, frame);
    return frame;
...
void x264_frame_reset_size( x264_t *h )
{
    int i_csp = x264_frame_internal_csp( h->param.i_csp );
    x264_frame_t *frame;
    int i_mb_count = h->mb.i_mb_count;
    ...
    int frame_num = 0;
    while ((frame = h->frames.framelist[frame_num++]) != NULL)
    {
        /* allocate frame data (+64 for extra data for me) */
        i_width  = h->mb.i_mb_width*16;
        i_lines  = h->mb.i_mb_height*16;
...
int x264_frame_copy_picture( x264_t *h, x264_frame_t *dst, x264_picture_t *src )
{
    ...
    dst->curr_level = src->curr_level;
    dst->ref_level  = src->ref_level;
...
x264_frame_t *x264_frame_pop_unused( x264_t *h, int b_fdec )
{
	...
	//fred
	frame->curr_level = tp_lvl_max;
	frame->ref_level  = tp_lvl_max;
...
int x264_sync_frame_list_init( x264_sync_frame_list_t *slist, int max_size )
{
    ...
    slist->i_size = 0;
    CHECKED_MALLOCZERO( x264_frame_t **,slist->list, (max_size+1) * sizeof(x264_frame_t*) );

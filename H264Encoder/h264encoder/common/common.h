...
#define LOG_M( format, ...) printf(format, __VA_ARGS__ );
#else
#define LOG_M( format, ...)
#endif
...
#define CHECKED_MALLOC(type, var, size )\
do {\
    var = (type)x264_malloc( size );\
    if( !var )\
        goto fail;\
} while( 0 )
#define CHECKED_MALLOCZERO( type,var, size )\
do {\
    CHECKED_MALLOC( type ,var, size );\
    memset( var, 0, size );\
} while( 0 )
...
//#define M128_ZERO ((x264_uint128_t){0,0})// by sensong
...
#define IS_SP( curr_level )              ( tp_lvl_1 == (curr_level))
#define IS_NORMAL_P( curr_level )        ( tp_lvl_1_p == (curr_level))
#define IS_LONG_REFFRAME( curr_level )   ( tp_lvl_2 == (curr_level))
#define IS_SHORT_REFFRAME( curr_level )  ( !IS_LONG_REFFRAME( curr_level ))
...
typedef struct x264_lookahead_t
{
	...
	x264_frame_t                  *backup_nonb;
	x264_frame_t                  *last_gf;
	x264_frame_t                  *last_sp;
...
typedef struct 
{
    uint16_t *g_cost_mv[QP_MAX+1];
    //uint16_t *g_cost_mv_fpel[QP_MAX+1][4];
    uint16_t g_x264_cost_ref[QP_MAX+1][3][33];
    int b_inited;
} encoder_common_data;

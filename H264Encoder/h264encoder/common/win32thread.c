...
int x264_pthread_mutex_lock( x264_pthread_mutex_t *mutex )
{
    ...
    if( !memcmp( mutex, &init, sizeof(x264_pthread_mutex_t) ) )
    {
        ...
        EnterCriticalSection(&thread_control.static_mutex);
    }
    else
    {
...
int x264_pthread_mutex_unlock( x264_pthread_mutex_t *mutex )
{
    ...
    {
        //*mutex = thread_control.static_mutex;
        LeaveCriticalSection(&thread_control.static_mutex);
    }
    else
    {
...
int x264_win32_threading_init( void )
{
    ...
    thread_control.cond_init = (void (WINAPI *)(x264_pthread_cond_t *))GetProcAddress((HMODULE) kernel_dll, "InitializeConditionVariable" );
    if( thread_control.cond_init )
    {
        /* we're on a windows 6.0+ kernel, acquire the rest of the functions */
        thread_control.cond_broadcast = (void (WINAPI *)(x264_pthread_cond_t *))GetProcAddress((HMODULE) kernel_dll, "WakeAllConditionVariable" );
        thread_control.cond_signal = (void (WINAPI *)(x264_pthread_cond_t *))GetProcAddress((HMODULE) kernel_dll, "WakeConditionVariable" );
        thread_control.cond_wait = (BOOL (__stdcall *)(x264_pthread_cond_t *,x264_pthread_mutex_t *,DWORD))GetProcAddress((HMODULE) kernel_dll, "SleepConditionVariableCS" );
    }

...
int ff_h264_frame_start(H264Context *h)
{
	...
	i = ff_MPV_frame_start( s, s->avctx );
	if( i < 0 )
		return -1;
...
static void decode_postinit(H264Context *h, int setup_finished)
{
	...
	assert( s->current_picture_ptr->sync );

	h->next_output_pic = s->current_picture_ptr;
...
static int decode_slice_header(H264Context *h, H264Context *h0)
{
	...
	if(	0==h->hasIDRflag )	// return -1 when miss idr
		return -1;
	...
	else
	{
		...
		//Add by landy Fix crash: When Some data is changed, this func might be used£ºNeed to fix
		if(s->me.qpel_avg == NULL)
			s->me.qpel_avg= s->dsp.put_h264_qpel_pixels_tab;
	...
	if (s->context_initialized 
		&&(h->coded_h264StandardFlag != h->h264StandardFlag /* added by sensong 20120531*/
	...
	if (!s->context_initialized) 
	{
		...
		// Start ======================================================
		h->coded_h264StandardFlag = h->h264StandardFlag;
		// End by sensong 20120529 ====================================
		...
		//av_assert0(s->avctx->sample_aspect_ratio.den);//return -1;
		if ( !(s->avctx->sample_aspect_ratio.den)) return -1;
	...
	{   // Shorten frame num gaps so we don't have to allocate reference frames just to throw them away
		...
		if(H264_MODE_STANDARD == h->h264StandardFlag)//added by sensong
		{
		//zhs note:
		// found frame_num gap, insert prev decoded frame
				...
				h->frame_num != (h->prev_frame_num + 1)%( 1<<h->sps.log2_max_frame_num ))
			{
                ...
                //zhs stores->current_picture_ptr(must be empty here)  in dpb
	...
	if(h->sps.poc_type==0){
		...
		if(h->pps.pic_order_present==1 && s->picture_structure==PICT_FRAME){
			h->delta_poc_bottom= get_se_golomb(&s->gb);							// delta_pic_order_cnt_bottom $se(v)
		}
	}

	if(h->sps.poc_type==1 && !h->sps.delta_pic_order_always_zero_flag){
		h->delta_poc[0]= get_se_golomb(&s->gb);									// delta_pic_order_cnt[ 0 ]   $se(v)

		if(h->pps.pic_order_present==1 && s->picture_structure==PICT_FRAME)
			h->delta_poc[1]= get_se_golomb(&s->gb);								// delta_pic_order_cnt[ 1 ]   $se(v)
	}

	init_poc(h);

	if(h->pps.redundant_pic_cnt_present){
		h->redundant_pic_count= get_ue_golomb(&s->gb);							// redundant_pic_cnt          $ue(v)
	}
	...
	if(h->slice_type_nos != AV_PICTURE_TYPE_I)
	{
		...
		if(h->slice_type_nos == AV_PICTURE_TYPE_B){
			h->direct_spatial_mv_pred= get_bits1(&s->gb);						// direct_spatial_mv_pred_flag       $u(1)
		}
		num_ref_idx_active_override_flag= get_bits1(&s->gb);					// num_ref_idx_active_override_flag  $u(1)

		if(num_ref_idx_active_override_flag)
		{
			h->ref_count[0]= get_ue_golomb(&s->gb) + 1;							// num_ref_idx_l0_active_minus1      $ue(v)
			if(h->slice_type_nos==AV_PICTURE_TYPE_B)
				h->ref_count[1]= get_ue_golomb(&s->gb) + 1;						// num_ref_idx_l1_active_minus1      $ue(v)
		}
	...
	//T-REC-H.264-201003-I!!PDF-E.pdf Page71
	//if( nal_ref_idc != 0 )
	//   dec_ref_pic_marking( )	
	...
	if( h->slice_type_nos != AV_PICTURE_TYPE_I && h->pps.cabac ){
		tmp = get_ue_golomb_31(&s->gb);													//cabac_init_idc            $ue(v)
		if(tmp > 2){
	...
	tmp = h->pps.init_qp + get_se_golomb(&s->gb);                                       //slice_qp_delta            $se(v)
	...
	if(H264_MODE_STANDARD == h->h264StandardFlag)
	{
		for(j=0; j<2; j++)
		{
			int id_list[16];
...
again:
			...
			case NAL_SPS:
				...
				if (ff_h264_decode_seq_parameter_set(h) < 0 && (h->is_avc ? (nalsize != consumed) && nalsize : 1)){
					...
					//if(h->is_avc) av_assert0(next_avc - buf_index + consumed == nalsize);//return -1;
					if (h->is_avc && !(next_avc - buf_index + consumed == nalsize)) return -1;
...
out_label:
		...
		out = h->delayed_pic[0];
		out_idx = 0;
		for ( i = 1; h->delayed_pic[i] && 0 == h->delayed_pic[i]->f.key_frame && 0 == h->delayed_pic[i]->mmco_reset; i++ )
		{
...
not_extra:
	...
	if ( NULL == s->current_picture_ptr && h->nal_unit_type == NAL_END_SEQUENCE) 
	{
		//av_assert0(buf_index <= buf_size);//return -1;
		if ( !(buf_index <= buf_size)) return -1;
		goto out_label;
	}
...
//    {"is_avc", "is avc", offsetof(H264Context, is_avc), FF_OPT_TYPE_INT, /*{.dbl = 0}*/ 0, 0, 1, 0},
//    {"nal_length_size", "nal_length_size", offsetof(H264Context, nal_length_size), FF_OPT_TYPE_INT, /*{.dbl = 0}*/ 0, 0, 4, 0},
//    {NULL}
//};
...
AVCodec ff_h264_decoder = 
{
	...
	CODEC_ID_H264,/*.id             = */
	/*CODEC_CAP_DRAW_HORIZ_BAND |*/ CODEC_CAP_DR1 | CODEC_CAP_DELAY, /* | CODEC_CAP_TRUNCATED |
																	 CODEC_CAP_SLICE_THREADS | CODEC_CAP_FRAME_THREADS.capabilities   = */
																	 ...
																	 &h264_class,/*const AVClass *priv_class = */
																	 NULL_IF_CONFIG_SMALL(profiles),/*const AVProfile *profiles*/
																	 sizeof(H264Context),  /*int priv_data_size = */
																	 NULL,/*struct AVCodec *next */
																	 //ONLY_IF_THREADS_ENABLED(decode_init_thread_copy),/*int (*init_thread_copy)(AVCodecContext *)*/
																	 //ONLY_IF_THREADS_ENABLED(decode_update_thread_context),/*int (*update_thread_context)(AVCodecContext *dst, const AVCodecContext *src) */
																	 //NULL, /*const AVCodecDefault *defaults */
																	 //NULL,/*void (*init_static_data)(struct AVCodec *codec) */
																	 ff_h264_decode_init,/*int (*init)(AVCodecContext *)= */
																	 //NULL,  /*int (*encode)(AVCodecContext *, uint8_t *buf, int buf_size, void *data) */  
																	 //NULL,/*int (*encode2)(AVCodecContext *avctx, AVPacket *avpkt, const AVFrame *frame, int *got_packet_ptr) */
																	 decode_frame,/*int (*decode)(AVCodecContext *, void *outdata, int *outdata_size, AVPacket *avpkt) = */
																	 ff_h264_decode_end,/*int (*close)(AVCodecContext *) = */

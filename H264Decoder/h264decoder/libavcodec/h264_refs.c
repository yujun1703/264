...
int ff_h264_fill_default_ref_list(H264Context *h)
{
	...
	if( AV_PICTURE_TYPE_B == h->slice_type_nos )
	{
		Picture *sorted[32];
		int cur_poc, list;
		int lens[2];
...
void ff_h264_remove_all_refs(H264Context *h){
	...
	for(i=0; i < 32 /*h->short_ref_count*/; i++)
	{
		if ( h->short_ref[i] )
...
zhs note:
1 do mmco on dpb
2 store current decoded frame into dpb(long or short)
3 s->current_picture_ptr may be decoded frame or empty( for frame_num gap )
*/
int ff_h264_execute_ref_pic_marking( H264Context *h, MMCO *mmco, int mmco_count )
{
	...
	for( i = 0; i < mmco_count; i++ )
	{
		...
		case MMCO_RESET:
			...
			h->frame_num = s->current_picture_ptr->frame_num = 0;
	...
	if ( 0 == current_ref_assigned )
	{
		...
		*/
		...
		else if ( 1 == s->current_picture_ptr->long_ref ) //zhs current decoded frame is long-ref 
		{
		...
		else // zhs current decoded frame is short-ref 
		{
			...
			//void* memmove(void* dest, const void* src, size_t cout);

			h->short_ref[0]= s->current_picture_ptr;//zhs store current frame into dpb
			h->short_ref_count++;
			s->current_picture_ptr->f.reference |= s->picture_structure;
		}
	}
